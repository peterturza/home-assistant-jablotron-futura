...
async def test_binary_sensors_created(hass: HomeAssistant):
    ...
    state = hass.states.get("binary_sensor.jablotron_futura_sn123456789_current_servo_drying")
    assert state is not None
    assert state.state == "off"  # False in mock data

    state = hass.states.get("binary_sensor.jablotron_futura_sn123456789_current_servo_bypass")
    assert state is not None
